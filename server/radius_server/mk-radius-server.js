...
server.on('message', (msg, rinfo) => {
  processPacket(msg, rinfo).then(() => {
    ...
    console.log(new Date(), err, 'errorrrrrrrrrrrrrrrrrrrrrrr');
  });
});
...
const processPacket = async (msg, rinfo) => {
  ...
  if (packet.code === 'Access-Request') {
    ...
    console.log(`${new Date()}: ${packet.code} of user ${username}`);
  }
  if (packet.code === 'Accounting-Request') {
    await sendAccountingResponde(packet, rinfo);
    console.log(`${new Date()}: ${packet.code} Acct-Status-Type: ${packet.attributes['Acct-Status-Type']}user: ${username}`);
  }
};
